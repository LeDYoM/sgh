...
namespace lib
{
	namespace draw
	{
		...
		public:
			...
			Transformation& operator=(const Transformation &rh) { m_matrix = rh.m_matrix; return *this; }

			Transformation(Transformation &&rh) : m_matrix( std::move(rh.m_matrix) ) {}
			Transformation& operator=(Transformation &&rh) { m_matrix = std::move(rh.m_matrix); return *this; }

			inline const TransformationDataType &matrix() const { return m_matrix; }
			inline TransformationDataType::const_pointer data() const { return m_matrix.data(); }
			...
			Transformation &operator*=(const Transformation &right) { return combine(right); }
			vector2df operator*(const vector2df &right) { return transformPoint(right); }
		private:
